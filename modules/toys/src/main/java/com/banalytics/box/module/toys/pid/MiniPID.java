 ...
 */
public class MiniPID implements PID {
    private double P = 0;
    private double I = 0;
    private double D = 0;
    private double F = 0;
    ...
    public MiniPID(String pidVal) {
        String[] parts = pidVal.split(";");
        P = Double.parseDouble(parts[0]);
        I = Double.parseDouble(parts[1]);
     ...
     *
     * @param targetPoint
     * @see MiniPID#getOutput(double, double) <br>
     */
    @Override
    public void setTargetPoint(double targetPoint) {
        this.setpoint = targetPoint;
    }
     ...
     * @param actual The monitored value, typically as a sensor input.
     * @return calculated output value for driving the system
     * @see MiniPID#setTargetPoint(double)
     */
